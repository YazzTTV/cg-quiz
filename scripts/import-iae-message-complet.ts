...
function parseMarkdownFile(filePath: string): QuestionData[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (questionMatch) {
      ...
      // Déterminer le texte de compréhension
      if (num >= 86 && num <= 100) {
        currentQuestion.comprehensionText = FRENCH_COMPREHENSION_TEXT
      } else if (num >= 156 && num <= 170) {
        currentQuestion.comprehensionText = ENGLISH_COMPREHENSION_TEXT
      }
      
      continue
    }
    
    // Détecter le prompt (ligne en gras ou ligne simple après ### Question)
    if (trimmed.startsWith('**') && currentQuestion && !currentQuestion.prompt) {
    ...
    // Si pas de prompt encore et que la ligne n'est pas vide et n'est pas un choix
    if (currentQuestion && !currentQuestion.prompt && trimmed && !trimmed.match(/^[*-]\s*\d+\./)) {
      // C'est probablement le prompt (ligne simple après ### Question)
      currentQuestion.prompt = trimmed
      continue
    }
    
    // Détecter les choix (lignes commençant par - ou * suivi d'un numéro)
    ...
    if (choiceMatch && currentQuestion) {
      ...
      // Nettoyer le texte (enlever ✅ et espaces)
...
async function main() {
  console.log('📝 Import des questions IAE Message complet...')
  
  const filePath = path.join(__dirname, '..', '..', 'iae_message_qcm_complet_avec_corrige.md')
      ...
      if (existing) {
        ...
        await prisma.question.create({
          data: {
            ...
            comprehensionText: qData.comprehensionText,
            status: 'APPROVED',
            source: 'IAE Message QCM complet',
            choices: {
              create: qData.choices
                .sort((a, b) => a.order - b.order)
                .map((choice) => ({
