  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (numberedMatch) {
      ...
      if (answers[num] !== undefined) {
        // Vérifier le contexte : regarder les lignes précédentes
        const prevLine = i > 0 ? lines[i - 1].trim() : ''
        const prevPrevLine = i > 1 ? lines[i - 2].trim() : ''
        
        // C'est une question si :
        // 1. Le numéro est > 5 (pas un choix)
        // 2. OU le texte est long (> 25 caractères) ET se termine par "?"
        // 3. OU le texte est long ET la ligne précédente était une question (numéro > 5)
